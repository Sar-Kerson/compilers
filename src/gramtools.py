...
#    follows[G.start].add('$')
#    #merge the frisr set into the follow set
#    for V in g_prods:
#        for each_prod in g_prods[V]:
#            if len(each_prod) > 0:
#                if each_prod[-1] in G.variables:
#                    for first in firsts:
#                        follows[V].add(first)
...
#        follows[v] = []
#    follows[G.start].append('$')
#    
#    total = 0
#    
#    while True:
#        for prds in G.productions:
#            for prd in G.productions[prds]:
#                prd = filter(lambda x: x != ' ', prd)
#                if len(prd) >= 2:
#                    for i in range(len(prd) - 1):
#                        if prd[i] in G.variables and prd[i + 1] in G.variables:
#                            for first in firsts[prd[i + 1]]:
#                                if first != '' and first not in follows[prd[i]]:
#                                    follows[prd[i]].append(first)
#        
#                    if len(prd) > 0:
#                        if prd[-1] in G.variables:
#                            for follow in follows[prds]:
#                                if follow not in follows[prd[-1]]:
#                                    follows[prd[-1]].append(follow)
#        print "@@" + str(follows) + "@@"   
#        temp = 0
#        for v in follows:
#            temp += len(follows[v])
